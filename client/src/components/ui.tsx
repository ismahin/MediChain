...
export function Button({ className = "", ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button className={`rounded-lg bg-medical-600 px-4 py-2 font-semibold text-white shadow-soft transition hover:bg-medical-700 disabled:cursor-not-allowed disabled:opacity-60 ${className}`} {...props} />;
}

export function Card({ className = "", ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={`rounded-lg border border-sky-100 bg-white p-5 shadow-soft ${className}`} {...props} />;
}

export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input className="w-full rounded-lg border border-sky-100 bg-white px-3 py-2 outline-none ring-medical-100 transition focus:ring-4" {...props} />;
}

export function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return <select className="w-full rounded-lg border border-sky-100 bg-white px-3 py-2 outline-none ring-medical-100 transition focus:ring-4" {...props} />;
}

export function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea className="w-full rounded-lg border border-sky-100 bg-white px-3 py-2 outline-none ring-medical-100 transition focus:ring-4" {...props} />;
}
