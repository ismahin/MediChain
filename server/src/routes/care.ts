...
async function hydrateCases(cases: any[]) {
  const users = await usersByIds(cases.flatMap((item) => [item.hospitalUserId, item.preferredDoctorUserId, item.assignedDoctorUserId, ...item.diagnosticOrders.map((order: any) => order.laboratoryUserId)]));
  return cases.map((item) => ({
    ...
    diagnosticOrders: item.diagnosticOrders.map((order: any) => ({ ...order, laboratory: users.get(order.laboratoryUserId) }))
  }));
}
...
router.post("/cases", requireRoles(Role.PATIENT), caseUpload.array("documents", 5) as any, asyncHandler(async (req, res) => {
  ...
  const careCase = await prisma.careCase.create({ data: { patientId: patient.id, problemTitle: body.problemTitle, problemDetails: body.problemDetails, hospitalUserId: hospital.id, preferredDoctorUserId: body.preferredDoctorUserId || null, appointmentDate, documents: { create: documents } }, include: { documents: true, diagnosticOrders: true } });
  await prisma.notification.create({ data: { userId: hospital.id, title: "New appointment request", message: `${req.user!.fullName} submitted ${body.problemTitle}.`, relatedEntityType: "CareCase", relatedEntityId: careCase.id } });
...
router.get("/cases/mine", requireRoles(Role.PATIENT), asyncHandler(async (req, res) => {
  const patient = await ownPatient(req.user!.id);
  const cases = await prisma.careCase.findMany({ where: { patientId: patient.id }, include: { documents: true, diagnosticOrders: true }, orderBy: { createdAt: "desc" } });
...
router.get("/documents/:id/download", asyncHandler(async (req, res) => {
  ...
  const allowed = req.user!.role === Role.ADMIN ||
    ...
    (req.user!.role === Role.DOCTOR && document.careCase.assignedDoctorUserId === req.user!.id && await hasPatientAccess(req.user!.id, document.careCase.patientId));
...
router.get("/hospital/cases", requireRoles(Role.HOSPITAL), asyncHandler(async (req, res) => {
  const cases = await prisma.careCase.findMany({ where: { hospitalUserId: req.user!.id }, include: { patient: { include: { user: { select: publicUserSelect } } }, documents: true, diagnosticOrders: true }, orderBy: { createdAt: "desc" } });
  ok(res, await hydrateCases(cases));
}));

router.get("/hospital/doctors", requireRoles(Role.HOSPITAL), asyncHandler(async (req, res) => {
  const memberships = await prisma.hospitalDoctor.findMany({ where: { hospitalUserId: req.user!.id }, select: { doctorUserId: true } });
...
router.post("/hospital/cases/:id/assign-doctor", requireRoles(Role.HOSPITAL), asyncHandler(async (req, res) => {
  ...
  if (!membership) throw new ApiError(400, "Add this doctor to Staff Doctors before assigning patient cases");
...
router.get("/doctor/cases", requireRoles(Role.DOCTOR), asyncHandler(async (req, res) => {
  const cases = await prisma.careCase.findMany({ where: { assignedDoctorUserId: req.user!.id }, include: { patient: { include: { user: { select: publicUserSelect } } }, documents: true, diagnosticOrders: true }, orderBy: { updatedAt: "desc" } });
...
router.post("/doctor/cases/:id/request-access", requireRoles(Role.DOCTOR), asyncHandler(async (req, res) => {
  ...
  const request = await prisma.accessRequest.create({ data: { patientId: careCase.patientId, requesterUserId: req.user!.id, requesterRole: Role.DOCTOR, requestedCategories: ["Full medical history", "Prescriptions only", "Diagnostic reports only"], reason: `Assigned care case: ${careCase.problemTitle}`, requestedDurationHours: 720 } });
...
router.post("/doctor/cases/:id/start", requireRoles(Role.DOCTOR), asyncHandler(async (req, res) => {
  ...
  if (!(await hasPatientAccess(req.user!.id, careCase.patientId))) throw new ApiError(403, "Patient must approve access before the session can start");
...
router.post("/doctor/cases/:id/diagnostic-orders", requireRoles(Role.DOCTOR), asyncHandler(async (req, res) => {
  ...
  if (!(await hasPatientAccess(req.user!.id, careCase.patientId))) throw new ApiError(403, "Active patient access is required");
...
router.post("/doctor/cases/:id/follow-up", requireRoles(Role.DOCTOR), asyncHandler(async (req, res) => {
  ...
  const updated = await prisma.careCase.update({ where: { id: careCase.id }, data: { doctorNotes: body.doctorNotes, followUpDate: body.followUpDate ? new Date(body.followUpDate) : null, status: body.completed ? CareCaseStatus.COMPLETED : CareCaseStatus.FOLLOW_UP } });
  const patient = await prisma.patientProfile.findUniqueOrThrow({ where: { id: careCase.patientId } });
...
router.get("/hospital/diagnostic-orders", requireRoles(Role.HOSPITAL), asyncHandler(async (req, res) => {
  const orders = await prisma.diagnosticOrder.findMany({ where: { hospitalUserId: req.user!.id }, include: { careCase: { include: { patient: { include: { user: { select: publicUserSelect } } } } } }, orderBy: { orderedAt: "desc" } });
  const users = await usersByIds(orders.flatMap((order) => [order.doctorUserId, order.laboratoryUserId]));
  ok(res, orders.map((order) => ({ ...order, doctor: users.get(order.doctorUserId), laboratory: order.laboratoryUserId ? users.get(order.laboratoryUserId) : undefined })));
}));
...
router.get("/laboratory/orders", requireRoles(Role.LABORATORY), asyncHandler(async (req, res) => {
  const orders = await prisma.diagnosticOrder.findMany({ where: { laboratoryUserId: req.user!.id }, include: { careCase: { include: { patient: { include: { user: { select: publicUserSelect } } } } } }, orderBy: { assignedAt: "desc" } });
  const users = await usersByIds(orders.map((order) => order.doctorUserId));
  ok(res, orders.map((order) => ({ ...order, doctor: users.get(order.doctorUserId) })));
}));
...
router.post("/laboratory/orders/:id/complete", requireRoles(Role.LABORATORY), reportUpload.single("file") as any, asyncHandler(async (req, res) => {
  ...
  const patient = await prisma.patientProfile.findUniqueOrThrow({ where: { id: order.patientId } });
  await Promise.all([
    prisma.notification.create({ data: { userId: patient.userId, title: "Diagnostic report completed", message: `Your ${order.testName} report is ready.`, relatedEntityType: "MedicalRecord", relatedEntityId: result.record.id } }),
    prisma.notification.create({ data: { userId: order.doctorUserId, title: "Diagnostic report completed", message: `${order.testName} results are ready for review.`, relatedEntityType: "MedicalRecord", relatedEntityId: result.record.id } })
  ]);
  ok(res, result, "Diagnostic report delivered to patient and doctor; anchor it with MetaMask");
}));
